...
async def call_me(query :types.CallbackQuery):
    ...
    phone = data[0][0]
    await query.message.answer(f'<b>Это Ваш верный номер телефона {phone}? '
...
async def text_me(query :types.CallbackQuery, state: FSMContext):
    await query.message.answer(text.DISPATCHER,
...
async def get_question_text(message: types.Message):
    await bot.forward_message(group_id, message_id=message.message_id, from_chat_id=message.chat.id)
...
async def yes(query: types.CallbackQuery, state: FSMContext):
    ...
    if current_data != {}:
        await bot.send_message(group_id, f'Пользователь @{query.from_user.username} оставил заявку на звонок по номеру {current_data["phone"]}')
        ...
        data = db.dict_factory('general', 'phone', ID)
        phone = data[0][0]
        await bot.send_message(group_id, f'Пользователь @{query.from_user.username} оставил заявку на звонок по номеру {phone}')
        await query.message.answer(text.ADMIN_CALL, parse_mode='HTML')
        await query.message.answer(text.GREETINGS, reply_markup=keyboards.main_keyboard())
        await state.finish()
        await query.answer()
